    ...
    void Update()
    {
        if (!DialogueAdded && GameManager.Instance.getPresentGamePhaseName().Equals("BadEndingFeedback"))
        {
            ...
            if (savedGameInfo.SelectedPrinterType == PrinterType.CartesianSmall)
            {
                ...
                dialogue.DialogueTextList.Add("릴리가 <color=#ff7777>엔진과 같은 대형 출력물</color>을 뽑을땐 오른쪽 프린터를 이용하라고 말해줬을텐데");
            }

            else if(savedGameInfo.SelectedPrinterType == PrinterType.Delta)
            {
                dialogue.DialogueTextList.Add("자네 델타 방식의 프린터를 이용했군");
                dialogue.DialogueTextList.Add("분명히 릴리가 <color=#ff7777>대형 출력물을 출력할땐 카르테시안 방식의 프린터</color>를 사용하라고 알려주었을텐데");
            }
            ...
            if(savedGameInfo.SelectedFilamentType == FilamentType.Engineer)
            {
                ...
                dialogue.DialogueTextList.Add("지금과 <color=#ff7777>같이 강도 높은 출력물을 출력할땐 엔지니어 필라멘트</color>를 사용해주게");
            ...
            else if (savedGameInfo.SelectedFilamentType != FilamentType.Engineer)
            {
                ...
                dialogue.DialogueTextList.Add("허허.. 실망이 크구만.  <color=#ff7777>강도가 높은 출력물을 출력할땐 엔지니어 필라멘트</color>를 사용했어야지..");
            }
